...
contract GensoMultisig {
    ...
    /// @notice NFT（ERC721）の出庫Proposalを作成する
    function proposeERC721Transfer(
        string memory _title,
        string memory _description,
        address _nft,
        uint256 _tokenId,
        ...
        require(_to != address(0), "GensoMultisig: zero address");
        require(_nft != address(0), "GensoMultisig: zero token");
        return
            _createProposal(
                ProposalType.ERC721Transfer,
                _title,
                _description,
                _to,
                _nft,
    ...
    function _erc721Transfer(address _nft, address _to, uint256 _tokenId) internal {
        // safeTransferFrom(address,address,uint256) を直接呼び出す
        (bool success, ) = _nft.call(
