...
`gh-pages` パッケージを同梱しているので、以下のコマンド1つで
ビルド→`gh-pages`ブランチへの公開までできます。

```bash
npm run deploy
```

初回はリポジトリの **Settings → Pages → Source** を「Deploy from a branch」、
ブランチを `gh-pages` / `/(root)` に設定してください
（GitHub Actionsを使う場合は前述の自動デプロイの方を利用し、こちらは不要です）。
